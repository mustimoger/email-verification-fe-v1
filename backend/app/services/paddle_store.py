...
def get_paddle_ids(user_id: str) -> Optional[Tuple[str, str]]:
    sb = get_supabase()
    try:
        ...
        if not data:
            return None
        row = data[0]
        return row["paddle_customer_id"], row["paddle_address_id"]
    except Exception as exc:  # noqa: BLE001
        logger.error("paddle_store.fetch_failed", extra={"user_id": user_id, "error": str(exc)})
        return None


def upsert_paddle_ids(user_id: str, customer_id: str, address_id: str) -> None:
    sb = get_supabase()
    try:
