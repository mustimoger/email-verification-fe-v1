    ...
    class FakeClient:
        async def download_task_results(self, task_id: str, file_format: str | None = None):
            ...
            return DownloadedFile(
                ...
                content_disposition='attachment; filename="task-1.csv"',
            )

    app = _build_app(monkeypatch, fake_user, FakeClient(), {"task_id": "task-1"})
    client = TestClient(app)
    resp = client.get("/api/tasks/task-1/download?format=csv")
    assert resp.status_code == 200
    assert resp.content == b"email,status\nfoo@example.com,exists\n"
    assert resp.headers["content-disposition"] == 'attachment; filename="task-1.csv"'
...
def test_tasks_download_requires_content_type(monkeypatch):
    ...
    app = _build_app(monkeypatch, fake_user, FakeClient(), {"task_id": "task-1"})
    client = TestClient(app)
    resp = client.get("/api/tasks/task-1/download")
