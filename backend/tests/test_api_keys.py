...
def test_resolve_user_api_key_creates_when_missing(monkeypatch):
    ...
    def fake_cache(user_id, key_id, name, key_plain=None):
        recorded.append((user_id, key_id, name, key_plain))
    ...
    assert recorded == [("user-1", "kid-123", "dashboard_api", "secret-123")]
...
def test_create_api_key_caches_secret(monkeypatch):
    ...
    app = _build_app(lambda: FakeClient(), fake_user)
    client = TestClient(app)
    resp = client.post("/api/api-keys", json={"name": "Zapier"})
    assert resp.status_code == 200
    assert recorded == [("user-4", "kid-new", "Zapier", "plain-secret")]
