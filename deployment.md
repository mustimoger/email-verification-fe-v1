...
- Latest state updates:
  ...
  - Step 100.1 execution attempt on `2026-02-08 17:47:49 UTC`: backup `/tmp/Caddyfile.backup.20260208T174749Z` and candidate `/tmp/Caddyfile.step1001.20260208T174749Z` were created; `caddy validate` passed and `caddy reload` succeeded from candidate; final write to `/etc/caddy/Caddyfile` failed with `Permission denied` due missing passwordless sudo.
  - Step 100.2 runtime smoke checks on `2026-02-08 17:48:29 UTC`: `boltroute.ai`, `www`, `/pricing`, `/integrations`, and `app.boltroute.ai/overview` all returned `HTTP/2 200`; DNS resolves to `135.181.160.203`; `boltroute-website` service is `active (running)`.
- Open items:
  - Persist website vhosts on disk in `/etc/caddy/Caddyfile` when root access is available; current routing is active from runtime reload only.
  - Re-run Step 100.2 checks immediately after successful on-disk persist to close the loop on durable config validation.
