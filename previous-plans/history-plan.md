...
- [x] Tests: verify current `/history` behavior (unfiltered list, refresh button, cache reuse, rows with null `api_key_id`).
  Explanation: The key selector was removed; tests should now cover the unfiltered list path, refresh behavior, cache reuse between navigations, and mapping of tasks even when `api_key_id` is null.
  Update: Added small helper guards for cache reuse and refresh gating, wired them into History, and extended history mapping tests to cover cache reuse, refresh gating, and null `api_key_id` mapping.
...
- [x] History cache: keep an in-memory cache so the page only refetches on manual refresh.
  Explanation: Added a user-scoped in-memory cache for History rows/total so navigating away and back reuses cached data; Refresh still triggers an external sync and updates the cache. Cache is cleared on sign-out.
