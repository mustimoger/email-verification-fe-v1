...
export const CONSENT_STORAGE_KEY = "consent.preference.v1";
export const CONSENT_VERSION = 1;
export const CONSENT_EVENT_NAME = "consent.updated";
...
const getLocalStorage = (): Storage | null => {
  if (typeof window !== "undefined") {
    try {
      ...
      console.warn("consent.storage_unavailable", { message });
      return null;
    }
  }
...
const parseConsentRecord = (raw: string): ConsentRecord | null => {
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    // eslint-disable-next-line no-console
    console.warn("consent.parse_failed", { message });
    return null;
  }
};
...
export const readConsentRecord = (): ConsentRecord | null => {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    // eslint-disable-next-line no-console
    console.warn("consent.storage_read_failed", { message });
    return null;
  }
  if (!raw) {
    return null;
  ...
  if (!record) {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      // eslint-disable-next-line no-console
      console.warn("consent.storage_clear_failed", { message });
    }
    // eslint-disable-next-line no-console
    console.warn("consent.record_invalid", { key: CONSENT_STORAGE_KEY });
    return null;
  }
  if (record.version !== CONSENT_VERSION) {
    ...
    // eslint-disable-next-line no-console
    console.info("consent.record_version_mismatch", {
...
const dispatchConsentUpdate = (update: ConsentUpdate) => {
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    // eslint-disable-next-line no-console
    console.warn("consent.event_dispatch_failed", { message });
  }
};

export const saveConsentDecision = (decision: ConsentDecision): ConsentUpdate => {
  ...
  if (storage) {
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      // eslint-disable-next-line no-console
      console.warn("consent.storage_write_failed", { message });
    }
  } else {
    // eslint-disable-next-line no-console
    console.warn("consent.storage_unavailable", { key: CONSENT_STORAGE_KEY });
  }
  ...
  if (persisted) {
    // eslint-disable-next-line no-console
    console.info("consent.decision_saved", {
      decision,
    ...
    // eslint-disable-next-line no-console
    console.warn("consent.decision_not_persisted", {
      decision,
      source: update.source,
    });
  }
