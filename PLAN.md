...
- [ ] Credit enforcement implementation — add ledger storage, atomic debit, idempotency, and wire to `/verify` + task completion with tests.  
  Explanation: Steps 2–3 complete — added `credit_ledger` table and an atomic `debit_credits` RPC + client helper (returns empty on insufficient). Remaining steps tracked in `credit-plan.md`.
