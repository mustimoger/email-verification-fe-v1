...
- [x] Step 10 — API Keys table: redacted key display with reveal + copy controls.
  Explanation: Added key reveal + copy controls in the API key column, passing through full keys when the external API provides them and falling back to `ext api data is not available` when they are missing (with console logs on reveal/copy attempts). New-key full values are cached client-side for the current session so the eye/copy controls work immediately after creation, and existing Revoke actions remain unchanged. Re-ran frontend tests (`npm run test:overview`, `npm run test:history`, `npm run test:auth-guard`, `npm run test:account-purchases`) with the Python venv active.
- [x] Step 11 — API Keys table spacing: add horizontal separation between API key + Integration columns.
...
- [x] Step 14 — API key modal: increase integration logo size (2×).
...
- [x] Step 17 — Validation card: add fixed-size status pills beside pie chart.
...
- [x] Overview backend alignment — Use credits-spent time series from Supabase tasks, add lifetime totals from external /metrics/verifications, and include latest Paddle plan + purchase date in /api/overview.
  Explanation: `/api/overview` now returns credits-spent totals/series from tasks, lifetime validation totals from external metrics (when available), and current plan data from `credit_grants` + `billing_plans`, including “Multiple items” when a purchase has multiple price_ids.
