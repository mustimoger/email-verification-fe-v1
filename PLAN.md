...
- [ ] Billing checkout conflict reuse — handle Paddle customer 409 by reusing existing customer reliably.
  Explanation: `/api/billing/transactions` returns 409 when Paddle customer creation conflicts and fallback lookup fails; update handler to import the correct response model and tolerate Paddle list/search response shapes so checkout can proceed.
