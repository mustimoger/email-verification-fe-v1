...
- [ ] Paddle checkout JS/CSP noise: pricing page initializes Paddle.js with backend status (sandbox/production) and checkout completes in sandbox. Remaining issues are report-only CSP warnings from Paddle/Sentry/Kaspersky and a Next/Image logo aspect ratio warning; consider swapping to `https://sandbox-cdn.paddle.com/paddle/v2/paddle.js` and adding CSS width/height:auto if cleanup is desired.
