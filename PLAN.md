...
- [ ] Paddle checkout still 409 on customer creation (sandbox): POST /customers returns 409; we now use deterministic email filter (`list_customers(email=...)`) with fuzzy fallback to reuse existing customers on conflict. Need to restart backend and retest checkout to confirm reuse and clear the 409 path. Added profile email backfill from auth claims during billing to avoid 400s for new users without a stored profile, and fixed Supabase profile upsert to work with current supabase-py API.
- [ ] Paddle checkout JS: pricing page now initializes Paddle.js with backend status (sandbox/production) to avoid sandbox transactions hitting live checkout; verify popup succeeds end-to-end (403 previously seen). Consider updating sandbox checkout script URL if needed.
