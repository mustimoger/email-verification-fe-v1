...
- [ ] Overview backend alignment — Use credits-spent time series from Supabase tasks, add lifetime totals from external /metrics/verifications, and include latest Paddle plan + purchase date in /api/overview.
  Explanation: Aligns Overview with real verification credit usage, lifetime validation totals, and billing-derived plan info without changing UI layouts.
