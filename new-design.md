...
### D3a: Dark-mode contrast fix for `/overview` secondary surfaces
- What: Improve readability for the secondary hero button and the verification history header/refresh controls in dark mode only.
- How: Introduce overview-scoped surface tokens and apply them to the affected elements so dark mode uses higher-contrast overlays without altering light mode styling.
- Why: The marked areas in dark mode were hard to read; this keeps the existing layout while making the text legible.
Status: Completed — dark-mode contrast fixes applied to the marked `/overview` elements without changing light mode visuals.
What was done and why:
- Added `--overview-surface-contrast` tokens to `app/overview-v2/overview-v2.module.css` so dark mode can use stronger overlays while light mode stays identical.
- Swapped the hero “View History” button, Verification history header row, and Refresh button to use the new tokens, improving readability in dark mode without altering layout or other components.

### D3b: Dark-mode contrast fix for `/overview` pagination controls
- What: Improve readability for the pagination buttons at the bottom right of the verification history table in dark mode only.
- How: Reuse the overview-scoped contrast surface token on the Prev/Next buttons so dark mode gets a stronger overlay without changing light mode styling.
- Why: The marked pagination buttons were too low-contrast in dark mode; this targets only those controls.
Status: Completed — pagination buttons now use the overview contrast surface token, improving dark-mode legibility without changing light mode visuals.
What was done and why:
- Updated the Prev/Next button backgrounds in `app/overview-v2/overview-v2-sections.tsx` to use `--overview-surface-contrast`, aligning them with the refreshed dark-mode overlay treatment.

### D3c: Add hover states to `/overview` hero buttons
- What: Add subtle hover feedback to the “Start Verification” and “View History” buttons.
- How: Introduce a small lift/glow on hover for the primary CTA and a light border/overlay change for the secondary button, keeping the existing layout and colors.
- Why: The hero CTAs currently lack hover affordance, making them feel static.
Status: Completed — hero buttons now include subtle hover affordances without layout changes.
What was done and why:
- Added a gentle lift + glow on the primary CTA and a border/overlay hover state on the secondary button in `app/overview-v2/overview-v2-sections.tsx` so hover feedback is visible but understated.
...
### D4b3: `/verify-v2` dark-mode contrast + hero hover polish
- What: Improve dark-mode readability for the red-marked `/verify` surfaces and add hover affordance to the hero CTAs.
- How: Introduce verify-scoped contrast surface tokens and apply them to the marked panels (manual input, live results, pre-flight checklist, bulk upload drop zone, workflow cards). Add the same hover effects used on `/overview` hero buttons.
- Why: The marked areas were hard to read in dark mode and the hero buttons lacked hover feedback.
Status: Completed — dark-mode contrast and hero hover states updated without changing layout.
What was done and why:
- Added `--verify-surface-contrast` tokens in `app/verify-v2/verify-v2.module.css` so dark mode can use stronger overlays while light mode stays unchanged.
- Swapped the red-marked panels (manual input container, live results rows + results list, pre-flight checklist items, bulk upload drop zone, workflow cards) to use the new contrast token for better legibility.
- Added the same hover lift/glow and secondary hover treatment from `/overview` to the hero “Start verification” and “View history” buttons.

### D4b4: `/verify-v2` dark-mode contrast for pills + export button
- What: Improve dark-mode readability for the “Auto-detect headers”, “Duplicate removal”, “Column mapping” pills and the “Export results” button.
- How: Use the verify contrast surface token for these controls so dark mode gets stronger overlays while keeping light mode unchanged.
- Why: These controls still looked too washed out in dark mode.
Status: Completed — pills and export button now use the contrast surface token for clearer dark-mode readability.
What was done and why:
- Updated the bulk upload pills and the “Export results” button in `app/verify-v2/verify-v2-sections.tsx` to use `--verify-surface-contrast`, keeping light mode unchanged while boosting dark-mode legibility.

### D4b5: `/verify-v2` hover states for manual action buttons
- What: Add subtle hover feedback to the “Verify emails” and “Clear input” buttons in the manual verification card.
- How: Reuse the hero CTA hover treatments (lift + glow for primary, border/overlay shift for secondary) without altering layout or colors at rest.
- Why: These buttons currently feel static compared to other CTAs.
Status: Completed — manual action buttons now mirror the hero CTA hover affordances.
What was done and why:
- Added the lift/glow hover to the manual “Verify emails” CTA and a border/overlay hover to “Clear input” in `app/verify-v2/verify-v2-sections.tsx`.
- Matched the same hover treatment on the bulk upload “Verify emails” CTA for consistency across the page.
...
### D4i1: `/history-v2` hero gradient overflow fix
- What: Remove the overflowed hero glow band showing on the right side of the history hero card.
- How: Match the `/overview` implementation by keeping glows scoped to the hero card, not the page container.
- Why: The gradient was applied to the wrong container, creating a visible overflow stripe in dark mode.
Status: Completed — removed page-level glows so the hero gradient is contained within the card.
What was done and why:
- Removed the page-level radial glow elements from `app/history-v2/history-v2-client.tsx`, leaving the hero-scoped glows in `HistoryHero` to match the `/overview` implementation and avoid overflow.

### D4i2: `/history-v2` dark-mode contrast + hero CTA hover polish
- What: Improve dark-mode readability for the red-marked history surfaces and add hover feedback to the hero CTAs.
- How: Introduce history-scoped contrast surface tokens and apply them to the marked panels (snapshot card body, export list rows, filter chips, refresh/load buttons, table container). Add the same hover lift/glow and secondary hover treatment used on `/overview` hero buttons.
- Why: The marked elements were too low-contrast in dark mode and the hero buttons lacked hover affordance.
Status: Completed — updated contrast surfaces and hero button hovers without changing layout.
What was done and why:
- Added `--history-surface-contrast` tokens in `app/history-v2/history-v2.module.css` to strengthen dark-mode overlays while keeping light mode unchanged.
- Swapped the marked history surfaces (snapshot summary body + stats cards, export list rows, filter chips, refresh/load buttons, table container) to use the new contrast tokens for better legibility.
- Added the same hover lift/glow and secondary hover treatment from `/overview` to the hero “Start verification” and “View API” buttons in `app/history-v2/history-v2-sections.tsx`.
...
### D4m1: `/integrations-v2` dark-mode contrast + hero CTA hover polish
- What: Improve dark-mode readability for the red-marked integrations surfaces and add hover feedback to the hero CTAs.
- How: Introduce integrations-scoped contrast surface tokens and apply them to the marked panels (integration cards, helper banner, usage list rows, quick-start steps). Add the same hover lift/glow and secondary hover treatment used on `/overview` hero buttons.
- Why: The marked elements were too low-contrast in dark mode and the hero buttons lacked hover affordance.
Status: Completed — updated contrast surfaces and hero button hovers without changing layout.
What was done and why:
- Added `--integrations-surface-contrast` tokens in `app/integrations-v2/integrations-v2.module.css` to strengthen dark-mode overlays while keeping light mode unchanged.
- Swapped the red-marked integrations surfaces (integration cards, helper banner, usage list rows, quick-start steps) to use the new contrast tokens for better legibility.
- Added the same hover lift/glow and secondary hover treatment from `/overview` to the hero “Generate API key” and “Start verifying” buttons in `app/integrations-v2/integrations-v2-sections.tsx`.
...
### D4s1: `/api-v2` dark-mode contrast + CTA hover polish
- What: Improve dark-mode readability for the red-marked API surfaces and add hover feedback to the CTA buttons.
- How: Introduce API-scoped contrast surface tokens and apply them to the marked panels (hero highlights, keys table, usage total pill, chart container, and outline CTAs). Add the same hover lift/glow and secondary hover treatment used on `/overview` hero buttons.
- Why: The marked elements were too low-contrast in dark mode and the CTA buttons lacked hover affordance.
Status: Completed — updated contrast surfaces and CTA hover states without changing layout.
What was done and why:
- Added `--api-surface-contrast` tokens in `app/api-v2/api-v2.module.css` to strengthen dark-mode overlays while keeping light mode unchanged.
- Swapped the red-marked API surfaces (hero highlights, keys table container/header, usage total pill, chart container, and outline CTAs) to use the new contrast tokens for better legibility.
- Added the same hover lift/glow and secondary hover treatment from `/overview` to “Generate API key”, “Review usage”, “See usage”, and “Download” buttons in `app/api-v2/api-v2-sections.tsx`.
- Removed the page-level radial glow layer in `app/api-v2/api-v2-client.tsx` so the hero glow stays contained (prevents the right-edge overflow stripe).
...
### D4aa: Swap `/account` to `/account-v2`
- What: Replace the existing `/account` route with the redesigned `/account-v2` UI.
- How: Point `app/account/page.tsx` to the v2 client while keeping `/account-v2` available for rollback.
- Why: Delivers the updated account design without changing backend behavior.
Status: Completed — `/account` now renders the `/account-v2` client.
What was done and why:
- Replaced `app/account/page.tsx` with a lightweight wrapper that renders `AccountV2Client` so the new design ships without altering backend behavior.
- Kept `/account-v2` intact for rollback/QA.
Tests:
- Not run (not requested).

### D4ab: `/account` dark-mode contrast fixes
- What: Fix the low-contrast surfaces highlighted in dark mode on `/account`.
- How: Add account-scoped contrast tokens and apply them to the red-marked buttons, pills, and cards.
- Why: Aligns `/account` dark mode readability with the fixes already applied on `/overview`.
Status: Completed — applied contrast tokens to the red-marked `/account` surfaces.
What was done and why:
- Added `--account-surface-contrast` and `--account-surface-contrast-strong` to `app/account-v2/account-v2.module.css`, mapping dark mode to the shared overlay tokens used on `/overview`.
- Swapped the hero secondary button, profile status pill, clear passwords button, purchase history table surfaces, and credits overview card to use the new contrast tokens so dark mode no longer uses bright white overlays.
Tests:
- Not run (not requested).

### D4ac: `/account` CTA label + hover parity
- What: Rename the hero CTA to "Verify emails", update its destination, and apply the Overview hover treatment to key account buttons.
- How: Mirror the `/overview` primary CTA hover classes for the hero CTA, profile update CTA, and clear passwords button.
- Why: Aligns `/account` CTA behavior with the approved `/overview` interaction polish.
Status: Completed — CTA label + hover parity aligned to `/overview`.
What was done and why:
- Updated the hero CTA to “Verify emails” pointing to `/verify`, and applied the same hover lift/glow as the `/overview` Start Verification button.
- Added the same hover lift/glow to the profile “Update profile” CTA and the outline hover treatment to “Clear passwords” so account buttons match the overview hover behavior.
Tests:
- Not run (not requested).

### D4ad: `/account` "View pricing" hover parity
- What: Add hover feedback to the hero "View pricing" button.
- How: Apply the same secondary hover treatment used on `/overview` (border accent, soft background, text shift).
- Why: Keeps the hero CTA pair consistent and improves dark-mode affordance.
Status: Completed — hover treatment matched to `/overview`.
What was done and why:
- Added the secondary hover lift, border accent, and soft background shift to the hero “View pricing” button so it matches the `/overview` secondary CTA behavior.
Tests:
- Not run (not requested).
